...
- [Architecture](#architecture) code structure & components
- [Prerequisites to launch the API](#prerequisites-to-launch-the-api)
...
Base URL (Docker): `http://localhost:8080`. Both endpoints live under `/api/v1`, return
JSON, and require a valid Keycloak **bearer token** carrying the right role (see
[Authentication & authorization](#authentication--authorization)). They are also subject to
[rate limiting](#rate-limiting).

The easiest way to try them is the **Swagger UI** at `/swagger`: click **Authorize**, sign
in, then **Try it out**. The `curl` snippets below assume a Keycloak access token in the
`$TOKEN` environment variable (signing in through Swagger is the simplest way to obtain one).

### `GET /api/v1/fizzbuzz`

Returns the numbers from `1` to `limit`, replacing multiples of `int1` with `str1`,
multiples of `int2` with `str2`, and multiples of both with the concatenation `str1str2`.
Requires the **`fizzbuzz.user`** role.


### `GET /api/v1/statistics`

Returns the most frequently submitted FizzBuzz request(s) and their hit count. Takes **no parameter**
if several requests tie for first place, all of them are returned. Requires the **`fizzbuzz.admin`** role.
...
The configuration lives in [`FizzBuzz/Presentation/Utils/RateLimiting.cs`](FizzBuzz/Presentation/Utils/RateLimiting.cs); the `fizzbuzz`
...
reflected by the endpoints.

## Architecture
### Code structure

The code is organized in layers with dependencies pointing **inward** (an onion / ports &
adapters style): outer layers depend on inner ones, never the reverse.

| Layer | Folder | Responsibility                                                                                          | Depends on |
|-------|--------|---------------------------------------------------------------------------------------------------------|------------|
| **Domain** | `Domain/` | Pure business logic: FizzBuzz evaluation. No framework, no I/O. | nothing |
| **Application** | `Application/` | Use cases that orchestrate the domain, plus the **ports** (interfaces) infrastructure must implement.   | Domain |
| **Infrastructure** | `Infrastructure/` | Adapters implementing the ports      | Application, Domain |
| **Presentation** | `Presentation/` | HTTP surface: Minimal API endpoints, OpenAPI / Swagger, rate limiting.                                  | Application, Domain |
| **Security** | `Security/` |Keycloak / OIDC authentication & authorization.                                           |  |

Concrete types are bound to their interfaces at the composition root, in
[`Program.cs`](FizzBuzz/Program.cs) and
[`DependencyInjection.cs`](FizzBuzz/DependencyInjection.cs).

### Components

Everything runs as a single `docker compose` stack:

| Service | Container | Host port | Role |
|---------|-----------|-----------|------|
| **FizzBuzz API** | `fizzbuzz` | `8080` | The ASP.NET Core service: the two endpoints, plus `/swagger` and `/health/*`. |
| **Redis** | `fizzbuzz-redis` | `6379` (localhost) | Request statistics, stored as a sorted set ranked by hit count. |
| **Keycloak** | `fizzbuzz-keycloak` | `8081` | Identity provider (OAuth2 / OIDC): authenticates users, signs the tokens the API and Grafana trust. |
| **OTel Collector** | `fizzbuzz-otel-collector` | internal | Receives the app's OTLP telemetry and fans it out to the backends. |
| **Aspire Dashboard** | `fizzbuzz-aspire` | `18888` | Zero-config dev view of logs + traces + metrics (in-memory). |
| **Grafana** | `fizzbuzz-grafana` | `3000` | Dashboards over the persistent backends; SSO via Keycloak. |
| **Prometheus** | `fizzbuzz-prometheus` | internal | Metrics store (scrapes the Collector). |
| **Tempo** | `fizzbuzz-tempo` | internal | Traces store. |
| **Loki** | `fizzbuzz-loki` | internal | Logs store. |
...
flowchart LR
    ...
    client -->|REST + Swagger| api
    client --> grafana
    client --> aspire
    client -.->|login OIDC| kc

    api -->|statistics| redis
    api -->|OTLP| collector
    api -->|validate JWT| kc
    grafana -->|OIDC SSO| kc

    collector -->|all signals| aspire
...
## Prerequisites to launch the API
...
`.env` holds the Keycloak admin credentials and the Grafana OAuth client
...
.env should not be commited but retrieved from a secured vault (e.g. AWS Secrets Manager).
however, here we're committing them for the sake of simplifying project onboarding.
These secrets only work locally and should not be reused in any production environment.
